...
## Agentic RL
传统的 RLHF(Reinforcement Learning from Human Feedback)需要大量人工标注偏好数据，成本高昂。为了降低成本，研究者提出了 RLAIF(Reinforcement Learning from AI Feedback)，用强大的 AI 模型(如 GPT-4)来替代人类标注员。RLAIF 的工作流程是:用 SFT 模型生成多个候选回答，用强大的 AI 模型对回答进行评分和排序，用 AI 的评分训练奖励模型，用奖励模型进行强化学习。实验表明，RLAIF 的效果接近甚至超过 RLHF，同时成本大幅降低Agentic RL 的关键特征是多步交互、每一步的行动都会改变环境状态、每一步都可以获得反馈、优化整个任务的完成质量。
...
| 核心能力                    | 简要说明                                               |
| ----------------------- | -------------------------------------------------- |
| 推理 (Reasoning)          | 从信息中逻辑得出结论。RL 通过试错学习有效推理策略，发现新推理路径，并学会何时深度思考或快速回答。 |
| 工具使用 (Tool Use)         | 调用外部工具完成任务。RL 让智能体学会何时、如何选择及组合工具（如计算器、代码解释器）。      |
| 记忆 (Memory)             | 保持并重用过去信息。RL 让智能体学会记忆管理：决定信息保留、更新或删除，类似人类工作记忆。     |
| 规划 (Planning)           | 制定行动序列达成目标。RL 支持动态规划，通过试错发现有效路径，权衡短期与长期收益。         |
| 自我改进 (Self-Improvement) | 回顾输出、纠正错误并优化策略。RL 让智能体学会自我反思与调整，实现持续改进。            |
| 感知 (Perception)         | 理解多模态信息。RL 可提升视觉推理能力，使智能体学会使用视觉工具并进行视觉规划。          |
![](https://hanphone.top/gh/HanphoneJan/public-pictures/agent/HelloAgents%20Agentic%20RL%20%E6%9E%B6%E6%9E%84.webp)
...
- `question`: 原始问题
- `prompt`: 输入提示(包含 system 和 user 消息)
- `ground_truth`: 正确答案，用于计算**准确率奖励**，判断模型生成的答案是否正确。
- `full_answer`: 完整答案(包含推理过程)，**不直接作为监督标签**，而是作为参考或辅助信息（例如用于初始化策略、提供对比示例，或作为基线）。最终训练时，RL 只根据 `question` + `prompt` 生成答案，与 `ground_truth` 比对。
...
| 奖励类型                      | 具体规则              | 组合策略                             | 特点     |
| ------------------------- | ----------------- | -------------------------------- | ------ |
| AccuracyReward（准确率奖励）     | 答案正确：+1<br>答案错误：0 | 准确率 + 长度惩罚<br>准确率 + 步骤奖励<br>三者平衡 | 基于准确率  |
| LengthPenaltyReward（长度惩罚） | 惩罚过长回答            | 准确率 + 长度惩罚<br>三者平衡               | 鼓励简洁   |
| StepReward（步骤奖励）          | 奖励清晰步骤            | 准确率 + 步骤奖励<br>三者平衡               | 鼓励详细推理 |
...
设计思路是:如果答案错误，奖励为 0(无论长度);如果答案正确且长度合理，奖励为 1;如果答案正确但过长，奖励为 1−α⋅(l−ltarget)

步骤奖励(StepReward)鼓励模型生成清晰的推理步骤，提高可解释性。数学定义为:

rstep(a,a∗,s)=racc(a,a∗)+β⋅srstep​(a,a∗,s)=racc​(a,a∗)+β⋅s

其中 ss 是检测到的推理步骤数量，ββ 是步骤奖励系数(默认 0.1)。同样，只有在答案正确的情况下才给予步骤奖励。
...
|  类别   |  指标                  |  解释                        |  优点      |  缺点       |     |
| :---- | :------------------- | :------------------------- | :------- | :-------- | --- |
|  准确性  |  Accuracy            |  模型输出与标准答案完全一致的比率          |  简单直观    |  过于粗糙     |     |
|  准确性  |  Accuracy@K          |  模型生成K个答案中任一正确即算正确，衡量上限潜力  |  反映潜力    |  需要多次采样   |     |
|  准确性  |  Numerical Error     |  数值结果与真实值的偏差程度（如绝对/相对误差）   |  细粒度     |  仅适用数值任务  |     |
|  效率   |  Avg Length          |  模型输出文本的平均长度，反映计算成本        |  反映成本    |  不考虑质量    |     |
|  效率   |  Avg Steps           |  推理过程中的平均步骤数，反映推理深度或风格     |  反映推理风格  |  难以量化     |     |
|  质量   |  Format Correctness  |  输出是否符合指定格式（如JSON、步骤标记）    |  易于检测    |  不保证正确性   |     |
|  质量   |  Coherence           |  输出逻辑一致性和可读性，需人工评估         |  全面评估    |  需要人工     |     |
#### 错误分析
模型的错误可以分为四类:计算错误(推理步骤正确但计算出错，如"48/2=25"，说明数值计算能力不足)、推理错误(推理逻辑错误导致解题思路不对，如先加后除而非先除后加，说明逻辑推理能力不足)、理解错误(没有正确理解问题，如问题问"总共"但只计算了一部分，说明语言理解能力不足)、格式错误(答案正确但格式不符合要求，如缺少"Final Answer:"标记，说明格式学习不足)。

### 超参数调优

| 方法    | 解释                         | 优点      | 缺点     | 适用场景      |
| :---- | :------------------------- | :------ | :----- | :-------- |
| 网格搜索  | 穷举指定超参数的所有组合，遍历全部候选点       | 简单，全局最优 | 计算成本高  | 参数少（2-3个） |
| 随机搜索  | 在超参数空间中随机采样组合进行评估          | 效率高     | 可能错过最优 | 参数多（4-6个） |
| 贝叶斯优化 | 基于概率代理模型，利用历史评估结果智能选择下一组参数 | 样本效率高   | 实现复杂   | 计算资源有限    |

### 分布式训练

| 方案名称             | 使用场景        | 核心原理 (怎么做的)                                                            | 显存占用              | 通信开销             | 典型命令/工具                                            |
| ---------------- | ----------- | ---------------------------------------------------------------------- | ----------------- | ---------------- | -------------------------------------------------- |
| **单机多卡 (DDP)**   | 2-8卡，简单高效   | 每张卡都复制一份完整的模型，各自吃一批数据，算完梯度后**互相通知**取平均，再统一更新。                          | **高** (模型+优化器全复制) | **中** (每步都要同步)   | `torchrun --nproc_per_node=N`                      |
| **大模型 (ZeRO-2)** | 模型 >7B，省显存  | 把优化器状态（如动量）切开，分散到不同卡上。每张卡只存1/N，用时再取。                                   | **中** (省掉优化器)     | **中** (需要取参数)    | `deepspeed --zero_stage=2`                         |
| **大模型 (ZeRO-3)** | 模型 >7B，更省显存 | 在ZeRO-2基础上，进一步把模型参数也切开。前向/反向计算前，临时从别的卡取参数，算完即丢。                        | **很低** (只存1/N参数)  | **高** (频繁取参数)    | `deepspeed --zero_stage=3`                         |
| **多节点集群**        | 多台机器，超大模型   | ZeRO-3 + 卸载(Offload) ，不仅把参数分到多张卡，还把暂时不用的数据**存到CPU内存甚至硬盘**，显卡只保留当前计算核心。 | **极低** (可跑千亿模型)   | **极高** (跨机器+内存慢) | `deepspeed --zero_stage=3 --offload_optimizer cpu` |
#### DDP（Distributed Data Parallel，分布式数据并行）
DDP 是 PyTorch 等框架中实现多机多卡分布式训练的主流方法。  
**核心思想**：将模型复制到每个 GPU 上，同时将一批数据切分成多个微批次，每个 GPU 处理不同的数据子集。  
**同步机制**：每个 GPU 独立完成前向和反向传播后，通过 **All-Reduce** 通信协议同步梯度，确保所有 GPU 上的模型参数始终一致。  
**特点**：通信量随模型参数量线性增长。比传统的 `DataParallel`（DP）更高效，因为减少了 GIL 竞争和主 GPU 瓶颈。

#### ZeRO（Zero Redundancy Optimizer，零冗余优化器）
ZeRO 是微软 DeepSpeed 框架提出的大模型训练优化技术，旨在消除数据并行中的冗余内存占用。  
**核心观察**：在标准 DDP 中，每个 GPU 都存储完整的模型参数、梯度和优化器状态，存在大量冗余。  
**优化阶段**（分三级）：
- **ZeRO-1**：只切分优化器状态（如 Adam 的一阶/二阶动量），参数和梯度仍全量保留。
- **ZeRO-2**：切分优化器状态 + 梯度。
- **ZeRO-3**：切分优化器状态 + 梯度 + 模型参数。训练时通过通信动态收集所需参数。  
#### Offload（卸载）
Offload 是一种**显存扩展技术**，将训练过程中不常访问的数据从 GPU 显存转移到 CPU 内存（甚至 NVMe 硬盘），需要时再换回 GPU。  
**典型应用**：与 ZeRO-3 结合（称为 ZeRO-Offload），将优化器状态和梯度卸载到 CPU，GPU 仅保留模型参数和计算中必需的临时数据。也可以单独用于卸载模型参数或激活值（如 Activation Offloading）。  
...
![](https://hanphone.top/gh/HanphoneJan/public-pictures/agent/BFCL%20%E8%AF%84%E4%BC%B0%E6%B5%81%E7%A8%8B%E5%9B%BE.webp)
...
**1. 精确匹配率 (Exact Match Rate)**：精确匹配率是 GAIA 的核心指标，定义为准精确匹配成功的样本比例
**2. 分级准确率 (Level-wise Accuracy)**
**3. 难度递进下降率 (Difficulty Progression Drop Rate)**：衡量智能体在难度增加时的性能衰减
...
GAIA 使用**准精确匹配（Quasi Exact Match）**算法作为官方评估标准。该算法的核心思想是：先对答案进行归一化处理，然后进行精确匹配。

给定预测答案 $A_{\text{pred}}$ 和标准答案 $A_{\text{true}}$，准精确匹配函数定义为：

$$
\text{Quasi\_Exact\_Match}(A_{\text{pred}}, A_{\text{true}}) = 
\begin{cases} 
1 & \text{if } N(A_{\text{pred}}) = N(A_{\text{true}}) \\
0 & \text{otherwise}
\end{cases}
$$

其中 $N(\cdot)$ 是归一化函数，根据答案类型应用不同的规则。归一化函数根据答案类型应用不同的规则：
- **数字类型** ：移除逗号分隔符（例如 `1,000` → `1000`），移除单位符号（例如 `$100` → `100`，`50%` → `50`）。  示例：`"$1,234.56"` 归一化为 `"1234.56"`。
- **字符串类型**：转换为小写（例如 `"Apple"` → `"apple"`），移除冠词（例如 `"the apple"` → `"apple"`），移除多余空格（例如 `"hello  world"` → `"hello world"`），移除末尾标点（例如 `"hello."` → `"hello"`）。  示例：`"The United States"` 归一化为 `"united states"`。
- **列表类型**：按逗号分隔元素，对每个元素应用字符串归一化，按字母顺序排序后重新连接。  示例：`"Paris, London, Berlin"` 归一化为 `"berlin,london,paris"`。
